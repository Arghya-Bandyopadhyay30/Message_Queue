...
int main() {
	...
	msgrcv(msgid1, &Number, sizeof(Number), 1, 0);
	
	int temp = atoi(Number.mtext);
	struct my_msgbuf Name[temp];
	msgrcv(msgid1, &Name, sizeof(Name), 2, 0);
	
	char** arr= malloc(sizeof(char*)*temp);
	int i;
    	for(i=0; i<temp; i++) {
        	arr[i]=malloc(sizeof(char)*10);
        	strcpy(arr[i], Name[i].mtext);
    	}
	MergeSort(arr, 0, temp-1);
	
	printf("Sorted Order Students:\n");
	for(int i=0; i<temp; i++) {
		strcpy(Name[i].mtext, arr[i]);
		printf("%s\n", Name[i].mtext);
	}
	...
	msgsnd(msgid3, &Name, sizeof(Name), 0);
}
